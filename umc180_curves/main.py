...
sim_var = ['model_n','model_p','temp','outputs','options','format','include','flavor','W', 'L','Ld','tran_extra' , 'VDS', 'VGS_ini', 'VGS_last', 'VGS_step','tempdata_filename','net_filename']
...
[m.write(var + '_' + flavor[0] + '\t= zeros(' + str(len(VGS)) + ',' + str(len(tran_data[0][0])) + ',' + str(len(tran_data[0])) + ',' + str(len(tran_data)) + ');\n') for var in outputs]
...
for j in range(sim_n):
	...
	m.write('\n')
	[m.write(var + '_'+ flavor[0] +'(:,' + str(i3+1) + ',' + str(i2+1) + ',' + str(i1+1) + ') = [' + ';'.join(eval(var + '_data')[i1][i2][i3]) + '];\n') for var in outputs]
